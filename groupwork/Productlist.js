...
let modal = document.querySelector("#modal"); //แถบสีขาว ตะกร้าสินค้า
let content = document.querySelector("#content"); //เนื้อหาข้อมูลที่อยู่ข้างในของ modal
let buy = document.querySelector("#buy"); //ปุ่ม 2 ปุ่ม ที่อยู่ใน modal 



//showcart
cartIcon.addEventListener('click', () => { //เมื่อ carticon ถูกคลิก จะใช้งาน function showcart
    modal.style.display = "block";
    showCart();

});

window.onclick = (event) => { //เช็คที่ target ไหนตรงกับ modal ไหม 
    if (event.target == modal) {
        clearViewCart(); //ถ้าตรงจะ clearViewCart
...
function showCart() {
    document.querySelector(".modal-content").style.visibility = "visible"; //ปรับให้เป็นมองเห็นได้
    setTimeout(() => {
        document.querySelector(".modal-content").style.width = "60%"; //โดยตอนแรกจะเป็น 50 แล้วเปลี่ยนเป็น 60 เพื่อทำเป็นอนิเมชั่น
    }, 1); //ให้รอเวลาแปปนึง


    if (cart.items.length > 0) {  //ถ้า cart item มี length ให้เช็คสินค้า
        cart.items.forEach((item, index) => {//ถ้า length มีค่ามากกว่า 0 
            content.innerHTML += ` 
            <tr> 
            <td>${index+1}</td>
        ...
        content.innerHTML = "ยังไม่มีสินค้า" //ถ้า length มีค่า = 0 
        buy.setAttribute("class", "d-none")
    }
}
buy.lastChild.addEventListener('click', () => { //มีปุ่มอยู่ 2 อัน อันสุดท้ายเป็น สีเขียว
    alert(`ขอบคุณที่อุดหนุนร้านอาหารแช่แข็งของเรา\nมูลค่าสุทธิ: ${cart.totalPrice} บาท`);
    clearViewCart(); // ให้เปลี่ยนจากการกด เป็น โชว์ alert
    canCel();
})
buy.firstChild.addEventListener('click', () => { //มีปุ่มอยู่ 2 อัน อันแรกเป็น สีแดง
    clearViewCart(); //ลบออกหมด
...
let theme = localStorage.getItem("theme"); //ให้เก็บค่าให้อยู่ใน local ของ theme
...
searchValue.addEventListener("keyup", () => {
    ...
    list(product);
    product.map((p) => {
        if ((p.name.match(value) == null)) { //ตรงกับตัวไหนให้โชว์ออกมา
...
function toggleSearch() {
    ...
    if (boolSearch) {
        searchValue.style.width = "0"; //ความชัดเป็น 0
        searchValue.style.visibility = "hidden" //มองไม่เห็น

    } else {
        searchValue.style.width = ""; //ความชัดให้มองได้ชัดเจน
        searchValue.style.visibility = "visible" //ทำให้มองเห็นได้
        searchValue.value = "";
    }
    list(product)
...
function list(product2 = product1) { 
    if (CookieUtil.get("cart") != null) { //get ว่ามี cookie ไหม
        cart = Cart.toCart(JSON.parse(CookieUtil.get("cart"))); //ถ้ามีให้เปลี่ยน cart แล้ว เปลี่ยน cart จากstringให้เป็นobj แล้วกลายเป็น Cart = cart
    }
    cal();
    foodlist.innerHTML = "";
    for (let p of product2) { //เปิด loop เพื่อเพิ่มlistอาหารอื่นๆ
        ...
        function alertAdd(event) {
            ...
            CookieUtil.set("cart", JSON.stringify(cart), Date('January 1, 2022')) //เปลี่ยน cart ให้กลายเป็น string ใน ver Json (กลับค่าคืน)
